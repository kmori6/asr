...
def main(config: DictConfig) -> None:
    ...
    pad_token_id, _, _ = validate_tokenizer(tokenizer, config.model.vocab_size)
    collate_fn = EncoderDecoderCollator(
        tokenizer=tokenizer,
        pad_token_id=pad_token_id,
        max_target_length=config.model.decoder.max_length,
    ...
    model = build_fast_conformer_transformer(config, blank_token_id=pad_token_id).to(device)
    optimizer = AdamW(
        model.parameters(),
