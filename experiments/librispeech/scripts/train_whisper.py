...
def main(config: DictConfig) -> None:
    ...
    if feature_extractor.sampling_rate != sample_rate:
        raise ValueError(
            f"Whisper feature extractor requires {feature_extractor.sampling_rate} Hz audio, "
    ...
    collator = EncoderDecoderCollator(
        ...
        max_target_length=int(model.config.max_target_positions),
    )
    ...
    logger.info(
        "Starting Whisper training: device=%s, bf16=%s, fp16=%s, parameters=%d",
        device,
        bf16,
        fp16,
    ...
    # Transformers 5.x does not currently include normalizer.json when saving
    # WhisperProcessor, so save the tokenizer vocabulary resources explicitly.
